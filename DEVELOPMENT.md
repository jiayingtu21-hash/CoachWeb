...
## v5 — Agent 页面 ChatGPT 风格重构 (commit `15f4b08`)

### 改动背景

原 Agent 聊天页面将 "新对话" 按钮和对话 ID 放在 Streamlit 左侧 sidebar 里，存在两个问题：
1. sidebar 里的内容会触发 Streamlit 自动在导航链接下方生成一条可展开的分割线（框架行为，CSS 无法完全隐藏）
2. 布局不够直观，不符合用户对 ChatGPT 式聊天界面的预期

### 改动内容

#### 1. `frontend/pages/5_🤖💬_Agent.py` — 完全重构

**布局变更：**
- 不再调用 `language_selector()`（不往 sidebar 写任何内容），改用 `init_language()` 仅初始化语言状态
- 顶栏三列布局：☰ 汉堡按钮（展开/收缩面板）+ 页面标题 + 🌐 中文/EN 语言切换
- 页面内两列布局（可收缩）：
  - **左侧面板 (22%)：** "＋ 开始新对话" 按钮 + 对话历史列表（从后端 API 加载）
  - **右侧聊天区 (78%)：** 消息气泡 + 底部输入框
- 面板收缩时（点击 ☰）聊天区域自动占满全宽

**新增功能：**
- `load_conversations()` — 调用 `/api/agent/conversations` 获取历史对话列表
- `load_history(conversation_id)` — 调用 `/api/agent/history/{id}` 加载完整对话历史
- 对话列表显示每个对话的第一条用户消息作为标题，点击可切换对话
- `show_conv_panel` session state 控制面板展开/收缩

**CSS 样式：**
- 深色主题面板样式（`.conv-panel`、`.conv-item`、`.new-conv-btn` 等）
- 滚动条美化、hover 高亮、active 状态边框

#### 2. `backend/services/storage.py` — `list_conversations()` 增强

```python
# 原来只返回 conversation_id, last_active, message_count
# 现在额外查询每个对话的第一条用户消息作为标题
first_user_msg = (
    db.query(ChatMessage.content)
    .filter(conversation_id=..., role="user")
    .order_by(ChatMessage.created_at)
    .first()
)
# 新增字段: "first_message": "用户的第一条消息..."
```

#### 3. `frontend/i18n.py` — 新增翻译键

| Key | 中文 | English |
|-----|------|---------|
| `agent_toggle_panel` | 展开/收起会话面板 | Toggle conversation panel |
| `agent_recent_conversations` | 💬 最近对话 | 💬 Recent Conversations |
| `agent_messages` | 条消息 | messages |
| `agent_no_conversations` | 还没有对话记录... | No conversations yet... |

### 已知限制

- Streamlit 1.30 sidebar 导航链接下方的可展开分割线是框架内置行为，无法通过 CSS 完全隐藏。不影响功能，保持现状。升级到 Streamlit 1.31+ 后可通过新的 sidebar API 解决。
